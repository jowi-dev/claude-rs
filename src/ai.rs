...
use crate::schema::ai_log::Entity;
use crate::schema::ai_log::Model as AILog;
use crate::db;
...
pub async fn get_logs() -> Result<Vec<AILog>, Box<dyn std::error::Error>> {
    let conn = db::connect().await?;
    let results : Vec<AILog> = Entity::find().all(&conn).await?;
